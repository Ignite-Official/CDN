...
async function check_email_exist() {
	if(!toggle_login_email_submit) {
		...
		ReactDOM.render(<Div_btn_submit_spinner class_btn={class_btn_enabled + " cursor-not-allowed"} text={"이메일로 본인 인증하기"} function={null} />, document.getElementById("btn_submit")) 
		...
		// 가입되어있지 않으면서 인증 대기중인 경우
			...
			ReactDOM.render(<Div_btn_submit class_btn={class_btn_enabled} text={"이메일로 본인 인증하기"} function={() => check_email_exist()} />, document.getElementById("btn_submit"))
		...
		// 가입되어있지 않으면서 인증이 만료된 경우
			...
			send_auth_email(email)      // 인증 메일 보내기
			ReactDOM.render(<Div_btn_submit class_btn={class_btn_enabled} text={"이메일로 본인 인증하기"} function={() => check_email_exist()} />, document.getElementById("btn_submit"))
			ReactDOM.render(<Div_desc_err_msg class={class_msg} text={"인증 메일이 발송되었습니다. (" + data.created_at + "에 재시도 가능)"} />, document.getElementById("desc_email_msg"))
		...
		// 회원 가입 진행
			...
			send_auth_email(email)      // 인증 메일 보내기
			ReactDOM.render(<Div_btn_submit class_btn={class_btn_enabled} text={"이메일로 본인 인증하기"} function={null} />, document.getElementById("btn_submit"))
			ReactDOM.render(<Div_desc_err_msg class={class_msg} text={"인증 메일이 발송되었습니다. (" + data.created_at + "에 재시도 가능)"} />, document.getElementById("desc_email_msg"))
