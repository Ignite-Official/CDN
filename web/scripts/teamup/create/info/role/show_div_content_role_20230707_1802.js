async function show_div_content_role() {
	...
	list_role_all = Object.values(Object.values(data_role)).map(x => x.uuid)        // 역할 UUID 목록
	list_instrument_all = Object.values(Object.values(data_instrument)).map(x => x.uuid)        // 악기 UUID 목록
	...
	if (list_cnt_role_selected.length > 0) {
		reopen_cnt_member()
	}
}
