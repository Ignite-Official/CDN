async function click_btn_submit()
{
	...
	if (!toggle_click_btn_submit) {
		...
		let data_active_region = Object.values(data_region).filter(it => it.active >= 1)  // 현재 active가 1 이상인 지역
		...
		if (txt_name == null || txt_name == "") {
			...
			alert("모집 마감 일자는 오늘보다 이전일 수 없습니다.");
			...
			const data = await fetch("/teamup/ajax_insert_teaminfo/", {
										method: "post", 
