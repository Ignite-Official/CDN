async function get_artist_list() {
	function Div_artist_sub(props) {
		const card_List = Object.keys(props.data).map((btn_data) =>                    
			<div class="flex flex-row w-full justify-center items-center hover:border hover:border-2 hover:border-gray-100 cursor-pointer"
				 onClick={() => location.href="/" + props.data[btn_data].uuid + ".me"}>
				<div class={    
								props.data[btn_data].url_image_bg == "Unknown"
								? "grid grid-cols-1 bg-gray-900 flex flex-col content-between items-between rounded-lg p-[16px] w-full max-w-[200px] h-[250px] min-h-[250px] max-h-[250px] mx-auto"
								: "grid grid-cols-1 bg-[linear-gradient(to_bottom,rgba(0,0,0,0),rgba(0,0,0,0.8)),url('" + props.data[btn_data].url_image_bg + "')] bg-cover bg-center flex flex-col content-between items-between rounded-lg p-[16px] w-full max-w-[200px] h-[250px] min-h-[250px] max-h-[250px] mx-auto"
						   }>
					{
