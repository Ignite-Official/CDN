...
function Div_role_list(props) {
	const btnList = Object.keys(props.data).map((btn_data) =>
		<button type="button" 
				id={"role_" + props.target + "_" +  props.data[btn_data].uuid} 
				name={"role_" + props.target + "_" + props.data[btn_data].uuid}
				onClick={() => click_btn_role(props.data[btn_data].uuid, props.target)}
