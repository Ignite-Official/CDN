...
function Div_region_list(props) {
	const btnList = Object.keys(props.data).map((btn_data) =>
		<button type="button" 
				id={"region_" + props.target + "_" +  props.data[btn_data].uuid} 
				name={"region_" + props.target + "_" + props.data[btn_data].uuid}
				onClick={() => click_btn_region(props.data[btn_data].uuid, props.target)}
