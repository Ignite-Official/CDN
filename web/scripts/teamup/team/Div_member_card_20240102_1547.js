function Div_member_card(props) {
	...
	function temp_click_left_btn_join_request_cancel(account, nickname) {
		ReactDOM.render(<Div_modal_join_request_cancel account={account} nickname={nickname} />, document.getElementById("modal_join_request_cancel"));       // 영입 요청 취소
		document.getElementById("btn_modal_join_request_cancel").click()
	}
	...
	return (
		<div class="flex flex-col justify-between items-start w-full space-y-[12px] bg-[#1D1D1D] p-[16px] rounded-lg">
			...
			{
				teamdata.teaminfo.my_type == "creator" && props.type == "sending"
				?    <div class="flex flex-between justify-center items-center w-full space-x-[4px]">
							  <p class="flex flex-row justify-center items-center bg-red-800 w-full h-[20px] rounded-lg
										text-white font-[600] text-[12px] cursor-pointer
										hover:bg-red-700 hover:border hover:border-white"
							  onClick = {() => temp_click_left_btn_join_request_cancel(props.account, props.nickname)}>
								   요청 취소
							  </p>
						 </div>
				:   ""
