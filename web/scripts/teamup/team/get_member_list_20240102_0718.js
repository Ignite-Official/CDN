async function get_member_list() {
  ...
  function Div_right_member_empty() {
		return (
  ...
  document.getElementById("tab_right_member_list").innerHTML = "현재 멤버(" + Object.keys(teamdata['member_list']).length + ")"
  if (Object.keys(teamdata['member_list']).length > 0) {
		ReactDOM.render(<Div_right_member_list data={teamdata['member_list']} />, document.getElementById("div_right_member_list"))
  } else {
		ReactDOM.render(<Div_right_member_empty />, document.getElementById("div_right_member_list"))
  }
